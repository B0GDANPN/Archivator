...
public:
    ...
    void encode(std::string &inputFilename) {
        ...
        //auto info = CommonInformation(static_cast<int>(header.subchunk2Size / size), duration.count(), size, header.subchunk2Size);
    ...
    void decode(const std::string &inputFilename) {
        auto start = std::chrono::high_resolution_clock::now();
        size_t pos = inputFilename.rfind('.');
        std::string outputFilename = inputFilename.substr(0, pos) + "Decoded.wav";// путь сохранения
        //std::ofstream outputFile(outputFilename, std::ios::binary);
...
private:
    ...
    bool readWAVHeader(const std::string& filename, WAVHeader& header/*, std::string& hStr*/) {
